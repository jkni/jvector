    ...
    public static DataSet create2DGrid(int nPoints, int nQueries, int topK) {
        ...
        for (int x = 0; x < gridWidth; x++) {
            for (int y = 0; y < gridWidth; y++) {
                baseVectors.add(vectorTypeSupport.createFloatType(new float[] {x, y}));
            }
        }
        ...
        var queries = IntStream.range(0, nQueries).parallel().mapToObj(i -> {
            ...
            Set<Integer> gt = IntStream.range(0, baseVectors.size())
                ...
                .sorted(Comparator.comparingDouble((Integer j) -> VectorSimilarityFunction.EUCLIDEAN.compare(vectorTypeSupport.createFloatType(q), baseVectors.get(j))).reversed())
                .limit(topK)
                .collect(Collectors.toSet());

            return new AbstractMap.SimpleEntry<>(vectorTypeSupport.createFloatType(q), gt);
